...
def time_to_seconds(m: int, s: int) -> int:
...
def extract_match_duration(raw_text: str) -> dict[str, int]:
    """Extrai duração de cada partida do RAW em segundos"""
    durations = {}
    for line in raw_text.splitlines():
        lower = line.strip().lower()
        # Procura por padrões como "(40 Min)", "(40:00)", "(58:20)", "(1:03)"
        m = re.search(r'\((\d{1,2}):?(\d{2})(?:\s*m(?:in)?)?\)', line, re.IGNORECASE)
        if m:
            mins = int(m.group(1))
            ...
            if lower.startswith("angel vs"):
                opponent = re.sub(r'\s*\([^)]*\).*', '', line.strip()[len("Angel vs"):]).strip()
                match_name = f"vs {opponent}"
            elif lower.startswith("angles vs"):
                opponent = re.sub(r'\s*\([^)]*\).*', '', line.strip()[len("Angles vs"):]).strip()
                match_name = f"vs {opponent}"
            else:
                continue
            
            durations[match_name] = total_secs
...
def parse_matches(raw_text: str) -> dict[str, pd.DataFrame]:
    ...
    for match_name, m_lines in matches_lines.items():
        pass_success = []
        pass_times = []  # Novo: armazenar os tempos dos passes
        coord_points = []
        for line in m_lines:
            ll = line.strip().lower()
            ...
            if tm:
                mins = int(tm.group(1))
                secs = int(tm.group(2))
                pass_times.append(time_to_seconds(mins, secs))
        ...
        for i in range(n_passes):
            ...
            passes.append({
                ...
                "time_seconds": pass_times[i] if i < len(pass_times) else 0
            })
...
def compute_stats(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "fwd": fwd_count,
        "back": back_count,
        "fwd_total_pct": fwd_pct_total
}

def compute_extended_stats(df: pd.DataFrame, match_duration_secs: int) -> dict:
    """Calcula estatísticas estendidas incluindo passes por 90 min e participação ativa"""
    base_stats = compute_stats(df)
    
    # Passes por 90 minutos
    passes_per_90 = (base_stats["total"] / match_duration_secs * 5400) if match_duration_secs > 0 else 0
    
    # Janela de participação ativa (primeiro e último passe)
    if len(df) > 0:
        first_pass_time = df["time_seconds"].min()
        last_pass_time = df["time_seconds"].max()
        active_window_secs = last_pass_time - first_pass_time
        active_window_str = seconds_to_mmss(active_window_secs)
        first_pass_str = seconds_to_mmss(first_pass_time)
        last_pass_str = seconds_to_mmss(last_pass_time)
        
        # Minutos ativos (tempo entre primeiro e último passe)
        active_minutes = active_window_secs / 60
        
        # Percentual de minutos participativos (minutos ativos / minutos jogados)
        participative_pct = (active_window_secs / match_duration_secs * 100) if match_duration_secs > 0 else 0
    else:
        active_window_secs = 0
        active_window_str = "0:00"
        first_pass_str = "0:00"
        last_pass_str = "0:00"
        active_minutes = 0
        participative_pct = 0
    ...
    return {
        **base_stats,
        ...
        "active_window_str": active_window_str,
        "first_pass_time": first_pass_time,
        "last_pass_time": last_pass_time,
        "first_pass_str": first_pass_str,
        "last_pass_str": last_pass_str,
        "active_minutes": active_minutes,
        "participative_pct": participative_pct,
        "match_duration_secs": match_duration_secs
    }
...
def draw_pass_map(df: pd.DataFrame, title: str) -> Image.Image:
    ...
    legend_elements = [
        ...
        Line2D([0], [0], color=(0.60, 0.60, 0.60, 0.65), lw=2.5, label="Completed Pass"),
    ]
    ax.legend(
...
match_durations = extract_match_duration(RAW)

df_all = pd.concat(matches_data.values(), ignore_index=True)
full_data = {"All games": df_all}
full_data.update(matches_data)

# Duplicar duração de todos os jogos para "All games"
all_games_duration = sum(match_durations.values())
...
# Determinamos a duração correta
if selected_match == "All games":
    match_duration_secs = all_games_duration
else:
    match_duration_secs = match_durations.get(selected_match, 0)

# Calculamos as estatísticas estendidas
stats = compute_extended_stats(df_sel, match_duration_secs)
...
def build_stats_table() -> pd.DataFrame:
    """Constrói a tabela com estatísticas por partida"""
    rows = []
    
    for match_name in list(matches_data.keys()):
        df_match = matches_data[match_name]
        duration_secs = match_durations.get(match_name, 0)
        extended_stats = compute_extended_stats(df_match, duration_secs)
        
        match_label = match_name
        total_passes = extended_stats["total"]
        
        # Conversão para minutos:segundos
        duration_mins = duration_secs // 60
        duration_secs_mod = duration_secs % 60
        duration_str = f"{duration_mins}:{duration_secs_mod:02d}"
        
        # Passes por 90 min (com 1 casa decimal)
        passes_per_90 = extended_stats["passes_per_90"]
        
        # Janela de participação ativa
        active_window = f"{extended_stats['first_pass_str']} até {extended_stats['last_pass_str']}"
        active_duration = extended_stats["active_window_str"]
        
        rows.append({
            "Partida": match_label,
            "Minutos Jogados": duration_str,
            "Total de Passes": total_passes,
            "Passes por 90 min": f"{passes_per_90:.1f}",
            "Janela de Participação Ativa*": f"{active_window} ({active_duration})"
        })
    
    return pd.DataFrame(rows)

def build_participation_table() -> pd.DataFrame:
    """Constrói a tabela de participação com minutos ativos"""
    rows = []
    ...
    for match_name in list(matches_data.keys()):
        ...
        active_secs = extended_stats["active_window_secs"]
        active_mins = active_secs / 60
        participative_pct = extended_stats["participative_pct"]
        
        rows.append({
            "Partida": match_name,
            "Minutos Jogados": duration_str,
            "Minutos Ativos (com passes)": f"{active_mins:.1f}",
            "% de Minutos Participativos": f"{participative_pct:.1f}%"
        })
    
    return pd.DataFrame(rows)

# Exibir tabelas
st.subheader("📊 Table 1 - Pass Statistics per Match")
df_stats_table = build_stats_table()
st.dataframe(df_stats_table, use_container_width=True, hide_index=True)

st.caption("* Janela de Participação Ativa: Período entre o primeiro e último passe do jogador")

st.subheader("📊 Table 2 - Active Participation Minutes")
df_participation_table = build_participation_table()
...
avg_passes = df_stats_table["Total de Passes"].astype(int).mean()
avg_passes_per_90 = df_stats_table["Passes por 90 min"].astype(float).mean()

# Calcular participação média
total_active_secs = 0
total_duration_secs = 0
for match_name in matches_data.keys():
    df_match = matches_data[match_name]
    duration_secs = match_durations.get(match_name, 0)
    extended_stats = compute_extended_stats(df_match, duration_secs)
    total_active_secs += extended_stats["active_window_secs"]
    total_duration_secs += duration_secs

avg_participation_pct = (total_active_secs / total_duration_secs * 100) if total_duration_secs > 0 else 0

col1, col2, col3, col4 = st.columns(4)
col1.metric("Average Passes per Match", f"{avg_passes:.1f}")
col2.metric("Average Passes per 90 min", f"{avg_passes_per_90:.1f}")
col3.metric("Avg. Participation %", f"{avg_participation_pct:.1f}%")
